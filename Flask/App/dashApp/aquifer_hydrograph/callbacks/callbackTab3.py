...
from App.dashApp.aquifer_hydrograph.myApp import app
from App.dashApp.aquifer_hydrograph.callbacks.data import *
...
def callbackTab3(app):
    ...
    def adjustment_aquifer_head(n_clicks, data, aquifer, threshold, mean_type):
        ...
        if len(aquifer) == 1:
            ...
            Final_Table['Index'] = abs(Final_Table['Delta']).apply(
            ...
            if len(n) > 0:
                while len(n) != 0:
                    ...
                    for i in range(n[0]):
                        ...
                        Final_Table['Delta'] = Final_Table['Adjusted_Aquifer_Head'].diff().fillna(
                            0)
                        Final_Table['Index'] = abs(Final_Table['Delta']).apply(
                            lambda x: 1 if x >= threshold else 0)
                        n = Final_Table.index[Final_Table['Index']
                                              == True].tolist()
            ...
            df_1 = pd.merge(left=df_1,
                            right=Final_Table[[
                                'Date_Gregorian', 'Date_Persian', 'Adjusted_Aquifer_Head']],
                            how='outer',
            ...
            title_name = 'متوسط تراز ماهانه (روز پانزدهم) سطح آب زیرزمینی در آبخوان ' + \
                aquifer[0]
        ...
        if len(aquifer) > 1:
            ...
            for aq in aquifer:
                ...
                Final_Table = df_aquifer[['Date_Gregorian', 'Date_Persian', 'Aquifer_Head', 'Aquifer_Head_Arithmetic_Mean', 'Aquifer_Head_Geometric_Mean', 'Aquifer_Head_Harmonic_Mean']].groupby(
                    by=['Date_Gregorian', 'Date_Persian']).mean().reset_index()
                Final_Table.replace(0, np.nan, inplace=True)
                Final_Table['Delta'] = Final_Table['Aquifer_Head'].diff().fillna(
                    0)
                Final_Table['Index'] = abs(Final_Table['Delta']).apply(
                    lambda x: 1 if x >= threshold else 0)
                Final_Table['Adjusted_Aquifer_Head'] = Final_Table['Aquifer_Head']
