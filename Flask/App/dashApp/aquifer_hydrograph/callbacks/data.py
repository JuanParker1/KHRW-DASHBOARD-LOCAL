...
import App.dashApp._assets.jalali as jalali
import statistics
...
def data_cleansing(well_info_data_all, dtw_data_all, thiessen_data_all, sc_data_all):
    ...
    for aquifer in well_info_data_all['نام آبخوان'].unique():
        ...
        Well_Info['نام آبخوان'] = Well_Info['نام آبخوان'].apply(
            lambda x: x.rstrip())
        ...
        dtw_data['نام آبخوان'] = dtw_data['نام آبخوان'].apply(
            lambda x: x.rstrip())
        ...
        thiessen_data['نام آبخوان'] = thiessen_data['نام آبخوان'].apply(
            lambda x: x.rstrip())
        thiessen_data['نام چاه'] = thiessen_data['نام چاه'].apply(
            lambda x: x.rstrip())
        ...
        sc_data['نام آبخوان'] = sc_data['نام آبخوان'].apply(
            lambda x: x.rstrip())
        ...
        # Modify Columns Name
        Storage_Coefficient_Wide.columns = [
            col for col in Storage_Coefficient_Wide.columns]
        ...
        Storage_Coefficient = pd.melt(frame=Storage_Coefficient_Wide,
                                      id_vars=['Date_Gregorian',
                                               'Date', 'Date_Persian'],
                                      value_name='Storage_Coefficient',
                                      var_name='ID').sort_values(['ID', 'Date_Gregorian']).drop('Date', axis=1)
        Storage_Coefficient = Storage_Coefficient[[
            'ID', 'Date_Gregorian', 'Date_Persian', 'Storage_Coefficient']]
        ...
        Elevation = Well_Info[[
            'کلاسه (کد شناسایی) چاه', 'G.S.L_M.S.L', 'Final Elevation', 'G.S.L_DEM_SRTM1']]
        # Elevation = Well_Info[['کلاسه (کد شناسایی) چاه', 'G.S.L_M.S.L', 'Final Elevation']]

        Elevation.columns = ['ID', 'MSL_Elevation',
        ...
        data['Unit_Aquifer_Storage_Coefficient'] = (
            data['Storage_Coefficient'] * data['Area']) / data['Aquifer_Area']
        ...
        data['Unit_Aquifer_Head'] = (
            data['Well_Head'] * data['Area']) / data['Aquifer_Area']
        ...
        df.columns = ['Date_Gregorian', 'Date_Persian', 'Aquifer_Head_Arithmetic_Mean',
                      'Aquifer_Head_Geometric_Mean', 'Aquifer_Head_Harmonic_Mean']
